...
!     Model parameters
      real(rk) :: muemax_phy,alpha,mortphy,rem,rkc, &
                  topt,tl1,tl2,depo,nbot,albedo_bio,drag_bio,w_det,w_phy
   contains
...
!  and are converted here to values per second.
   ...
   call self%get_parameter(self%w_phy, 'w_phy', default=w_phy, scale_factor=secs_pr_day)
   call self%get_parameter(self%w_det, 'w_det', default=w_det, scale_factor=secs_pr_day)
...
!  Register state variables
   ...
   call self%register_state_variable(self%id_phy,'phy','mmol/m**3','phytoplankton', &
                                    vertical_movement=w_phy/secs_pr_day    &
                                    ,specific_light_extinction=rkc)
